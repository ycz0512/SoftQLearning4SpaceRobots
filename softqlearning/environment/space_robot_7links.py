...
class SpaceRobot3link(object):
    ...
    def _get_handles(self):       # get handles of objects in V-REP
        joint_names = ['Sawyer_joint1', 'Sawyer_joint2', 'Sawyer_joint3', 'Sawyer_joint4',
                       'Sawyer_joint5', 'Sawyer_joint6', 'Sawyer_joint7', 'Gjoint_1', 'Gjoint_2']     # object joint names in V-REP
    ...
    def _get_observation(self):     # get current observations s_t
        ...
        # state is [θ1, θ2, θ3, θ4, θ5, θ6, θ7, ω1, ω2, ω3, ω4, ω5, ω6, ω7,
        #           Base_α, Base_β, Base_γ, Base_x, Base_y, Base_z, Vx, Vy, Vz, dα, dβ, dγ]

        Joint_angle = [vrep.simxGetJointPosition(self.clientID, self.handles['joint'][i], vrep.simx_opmode_blocking)[1]
                       for i in range(7)]      # retrieve the rotation angle as [θ1, θ2, θ3, θ4, θ5, θ6, θ7]
        state[0: 7] = Joint_angle

        Joint_velocity = [vrep.simxGetObjectFloatParameter(self.clientID, self.handles['joint'][i], 2012, vrep.simx_opmode_blocking)[1]
                          for i in range(7)]        # retrieve the joint velocity as [ω1, ω2, ω3, ω4, ω5, ω6, ω7]
        state[7: 14] = Joint_velocity

        Base_pose = [vrep.simxGetObjectOrientation(self.clientID, self.handles['base'][0],
                                                   -1, vrep.simx_opmode_blocking)[1]]
        state[14: 17] = Base_pose[0]          # retrieve the orientation of Base as [Base_α, Base_β, Base_γ]

        Base_position = [vrep.simxGetObjectPosition(self.clientID, self.handles['base'][0],
                                                    -1, vrep.simx_opmode_blocking)[1]]
        state[17: 20] = Base_position[0]     # retrieve the position of Base as [Base_x, Base_y, Base_z]

        _, Base_Vel, Base_Ang_Vel = vrep.simxGetObjectVelocity(self.clientID, self.handles['base'][0],
                                                               vrep.simx_opmode_blocking)
        state[20: 23] = Base_Vel        # retrieve the linear velocity of Base as [Vx, Vy, Vz]
        state[23: 26] = Base_Ang_Vel        # retrieve the angular velocity of Base as [dα, dβ, dγ]

        state = np.asarray(state)

        return state

    def _set_joint_effort(self, U):         # set torque U = [u1, u2, u3, u4, u5, u6, u7] to 7 joints in V-REP
