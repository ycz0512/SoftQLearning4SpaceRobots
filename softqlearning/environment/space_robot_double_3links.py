...
class SpaceRobot3link(object):
    ...
    def _get_handles(self):       # get handles of objects in V-REP
        joint_names = ['Joint_1', 'Joint_2', 'Joint_3', 'Joint_4', 'Joint_5', 'Joint_6',
                       'Gjoint_1', 'Gjoint_2', 'Gjoint_3', 'Gjoint_4']     # object joint names in V-REP
        ...
        point_names = ['EE_point_1', 'EE_target_point1', 'EE_point_2', 'EE_target_point2']      # object EE_point names in V-REP
    ...
    def _set_joint_effort(self, U):         # set torque U = [u1, u2, u3, u4, u5, u6] to 6 joints in V-REP
        torque = [vrep.simxGetJointForce(self.clientID, self.handles['joint'][i], vrep.simx_opmode_blocking)[1]
                  for i in range(self.action_dims)]     # retrieve the current torque from the joints as [jt1, jt2, jt3, jt4, jt5, jt6]
    ...
    def grip1(self):     # gripper 1 close
        for i in range(6, 8):
    ...
    def grip2(self):     # gripper 2 close
        for i in range(8, 10):
            _ = vrep.simxSetJointTargetPosition(self.clientID, self.handles['joint'][i], 0, vrep.simx_opmode_blocking)
            # Just in case
            if _ != 0:
    ...
    def _reward(self, U):       # get the current reward
        ...
        EE1_position = vrep.simxGetObjectPosition(self.clientID, self.handles['points'][0],
                                                  -1, vrep.simx_opmode_blocking)[1]
        EE2_position = vrep.simxGetObjectPosition(self.clientID, self.handles['points'][2],
                                                  -1, vrep.simx_opmode_blocking)[1]
        EE_target1_position = vrep.simxGetObjectPosition(self.clientID, self.handles['points'][1],
                                                         -1, vrep.simx_opmode_blocking)[1]
        EE_target2_position = vrep.simxGetObjectPosition(self.clientID, self.handles['points'][3],
                                                         -1, vrep.simx_opmode_blocking)[1]
        distance1_square = np.sum(np.subtract(EE1_position, EE_target1_position)**2)
        distance2_square = np.sum(np.subtract(EE2_position, EE_target2_position)**2)

        # TODO: define distance loss
        distance_square = distance1_square + distance2_square
